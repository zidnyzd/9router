...
describe("OpenCode Stable Session Reuse (429 follow-up)", () => {
  ...
  it("injects the upstream-mandated file-search quartet into free-tier requests", () => {
    const executor = getExecutor("opencode");

    // Case 1: no tools sent by client -> injects full quartet with tool_choice none
    ...
    // Case 2: external CLI tools (e.g. Claude Code Bash) -> preserved, missing quartet merged
    ...
    // Case 3: already has full quartet -> do not insert anything
    ...
    // Case 4: Responses path injects quartet even when caller sent tools (no 403 gap)
    const responsesWithTools = executor.transformRequest("muse-spark-1.3-contributor-free", {
      input: [{ type: "message", role: "user", content: [{ type: "input_text", text: "hi" }] }],
      tools: [{ type: "function", name: "my_tool", description: "caller", parameters: { type: "object", properties: {} } }],
    });
    expect(responsesWithTools.stream).toBe(true);
    expect(responsesWithTools.store).toBe(false);
    const responseNames = responsesWithTools.tools.map((t) => t.name);
    expect(responseNames[0]).toBe("my_tool");
    for (const required of ["bash", "glob", "grep", "read"]) {
      expect(responseNames).toContain(required);
    }

    // Case 5: Responses path with no tools -> full quartet
    const responsesNoTools = executor.transformRequest("muse-spark-1.3-contributor-free", {
      input: [{ type: "message", role: "user", content: [{ type: "input_text", text: "hi" }] }],
    });
    expect(responsesNoTools.tools.map((t) => t.name)).toEqual(["bash", "glob", "grep", "read"]);
  });
